  ...
  async (): Promise<{ week: GroupedIssues[]; month: GroupedIssues[] }> => {
    try {
      ...
      const {
        ...
        LastWeekOfCurrentMonth,
      } = getWeekAndMonthDates();
      const filteredIssuesForWeek = filterIssuesByDate(
        issues,
        startOfWeek,
        endOfWeek
      );
      const filteredIssuesForMonth = filterIssuesByDate(
        issues,
        LastWeekOfPreviousMonth,
        LastWeekOfCurrentMonth
      );
      const groupedIssuesForWeek = groupIssues(
        filteredIssuesForWeek,
        startOfWeek,
        7
      );
      const groupedIssuesForMonth = groupIssues(
        filteredIssuesForMonth,
        LastWeekOfPreviousMonth,
        35
      );
      const groupedIssuesArrayForWeek: GroupedIssues[] = Object.keys(
        groupedIssuesForWeek
      ).map((date) => ({
        day: date,
      ...
      const groupedIssuesArrayForMonth: GroupedIssues[] = Object.keys(
        groupedIssuesForMonth
      ).map((date) => ({
        day: date,
      ...
      return {
        week: groupedIssuesArrayForWeek,
        month: groupedIssuesArrayForMonth,
      };
    } catch (error) {
