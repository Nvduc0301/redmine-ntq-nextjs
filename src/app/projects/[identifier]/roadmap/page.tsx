...
const Roadmap: React.FC<RoadmapProps> = ({ identifier }) => {
  ...
  return (
    <div>
      ...
      {loading ? ( // Conditional rendering for loading state
        ...
        projectVersions
          .filter(
            (projectVersion) => showClosed || projectVersion.status === 'open'
          )
          .map((projectVersion) => {
                  ...
                  <div>
                    ...
                    <table className="mt-3 w-full">
                      ...
                      <tbody>
                        {versionData.issues
                          .filter(
                            (issue) => showBug || issue.tracker.name !== 'Bug'
                          )
                          .filter(
                            (issue) => showTask || issue.tracker.name !== 'Task'
                          )
                          .map((issue) => (
                            <tr key={issue.id}>
                              <td className="flex items-center border border-[#cccccc] max-w-full">
                                ...
                                <div className="text-xs text-[#484848]">
                                  : {issue.subject}
                                </div>
                              </td>
                            </tr>
