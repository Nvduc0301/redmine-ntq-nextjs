import { axiosInstance, axiosPost, uploadFile } from './api';
import { Issue, IssueData, IssueReport } from '../types/Issue';
import { fetchAPIGet } from '~/utils/helperAPI';

export const getIssueSchedule = async (): Promise<Issue[]> => {
  try {
...
export const getIssueReport = async (): Promise<IssueReport[]> => {
  try {
    const response = await axiosInstance.get<{ issues: Issue[] }>(
      '/issues.json?author_id=me'
    );
